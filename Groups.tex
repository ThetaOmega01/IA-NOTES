...
\begin{document}
    ...
    \section{Basic properties}
        ...
        \end{remark}
        \begin{definition}
            The \textit{kernel} of an action $ \alpha:G \times X \to X $ is the kernel of the above $ \rho $.\marginnote{All the permutations of $G$ that act as the identity of $ \sym(X) $. i.e., they do nothing to $x$.}
        \end{definition}
        Note that $ G/\ker \rho \cong \im \rho\le \sym(X) $ by theorem \ref{thm:1st isom thm}. In particular, if $ \ker \rho = \left\{ e\right\} $, then $ G\le \sym(X) $.
