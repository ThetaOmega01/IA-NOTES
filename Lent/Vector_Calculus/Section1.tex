...
\]
so for regular curves we have $ \rmd s/\rmd t\>0 $, so we can invert relationship between $s$ and $t$ to find $t=t(s)$. So we can parametrise \textit{regular} curves with respect to arc-length. Write $ \mathbf{r}(s)=\mathbf{x}(t(s)) $ where $ 0\le s\le \ell (C) $, then by chain rule, 
\[
...
\begin{definition}[Principal normal]
    The \textbf{principal normal} is defined by 
    \[
        \bft'=\kappa \bfn.
    \]
