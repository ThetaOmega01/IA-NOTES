...
\begin{definition}[Probability Space]
    Suppose $ \Omega $ is a set(called \textbf{sample space}) and $ \mathscr{F} $ is a collection of subsets of $ \Omega $. We call $\mathcal{F}$ a \textbf{$\sigma$-algebra} if 
    \begin{enumerate}
        \item $ \Omega\in \mathscr{F} $,
        \item $ A\in \mathscr{F} \Rightarrow A^\complement \in \mathscr{F} $,
        \item For any \textit{countable} collection $ (A_n)_{n\ge 1} $ with $A_n\in \mathcal{F}$, $ \bigcup_{n=1}^{\infty}A_n\in \mathscr{F} $.
    \end{enumerate}
    Suppose $\mathcal{F}$ is a $ \sigma $-algebra on $ \Omega $. A function $ \mathbb{P}:\mathscr{F}\to [0,1] $ is called a \textbf{probability measure} if
    \begin{enumerate}
...
\end{definition}
\begin{remark}
    When $ \Omega $ is countable, we take $\mathcal{F}$ be all subsets of $ \Omega $.
\end{remark}
\begin{definition}[Outcomes, Events]
    The elements of $ \Omega $ are called \textbf{outcomes} and the elements of $\mathcal{F}$ are called \textbf{events}.
\end{definition}
\begin{remark}
    We talk about probabilities of \textit{events} rather than \textit{outcomes}.
\end{remark}
\begin{proposition}[Properties of $\mathbb{P}$]\label{prop:Properties of P}
...
\end{example}
\begin{example}
    \underline{Equally likely outcomes.} Let $ \Omega = \{\omega_1,\dots, \omega_n\} $ be a finite set and $ \mathscr{F}=2^\Omega $. Define $\mathbb{P}$ as $ \mathbb{P}(A)=|A|/|\Omega| $. In classical probability, this models picking a random element of $ \Omega $. Note that $ \mathbb{P}(\{\omega\})=1/|\Omega|, \forall \omega\in \Omega $.
\end{example}
\begin{example}
...
\begin{proof}
    \begin{align*}
        \mathbb{P}(\cup A_n|B) &= \frac{\mathbb{P}((\cup A_n)\cap B)}{\mathbb{P}(B)} = \frac{\mathbb{P}(\cup(A_n\cap B))}{\mathbb{P}(B)}\\ 
        &=\sum_{n=1}^{\infty}\frac{\mathbb{P}(A_n\cap B)}{\mathbb{P}(B)}=\sum_{n=1}^{\infty}\mathbb{P}(A_n|B).
    \end{align*}
\end{proof}
...
\begin{proof}
    \[
        \mathbb{P}(B_n|A) = \frac{\mathbb{P}(B_n\cap A)}{\mathbb{P}(A)}= \frac{\mathbb{P}(A|B_n)\mathbb{P}(B_n)}{\sum_{k=1}^{\infty}\mathbb{P}(A|B_k)\mathbb{P}(B_k)}.
    \]
\end{proof}
...
\begin{example}[False positive for a rare condition]
    ...
    \]
    want to find $\mathbb{P}(A|P)$. Note that $ \mathbb{P}(A)=0.001, \mathbb{P}(P|A) = 0.98, \mathbb{P}(P|A^\complement)=0.01 $. By Bayes' formula,
    \[
...
\begin{example}[Simpson's paradox]
    ...
    \begin{align*}
        ...
        &= \frac{\mathbb{P}(A\cap B\cap C)}{\mathbb{P}(C)}+\frac{\mathbb{P}(A\cap B^\complement\cap C)}{\mathbb{P}(C)}\\ 
